I need you to build a new IV Percentile Mean Reversion strategy and clean up my options trading system by removing all broken/blocked strategies. Here's exactly what I want:

## PART 1: CREATE NEW IV PERCENTILE STRATEGY

Create a new file called `iv_percentile_strategy.py` with this exact implementation:
...
from paper_trader import book_signal
...
class IVPercentileStrategy:
    ...
    def get_iv_percentile(self, security_id: str, current_iv: float) -> float:
        ...
        if len(ivs) < CONFIG["MIN_HISTORY_DAYS"]:
            return 50.0
    ...
    def _build_sell_signal(self, security_id, symbol, spot, current_iv, percentile, zscore, trend):
        ...
        for strike in strikes:
            ...
            if opt:
                ...
                if CONFIG["STRANGLE_DELTA_TARGET"] - 0.05 <= delta <= CONFIG["STRANGLE_DELTA_TARGET"] + 0.05:
                    ce_leg = (strike, opt)
                    break
        ...
        if ce_oi < CONFIG["MIN_OI"] or pe_oi < CONFIG["MIN_OI"]:
            return None
        if ce_vol < CONFIG["MIN_VOLUME"] or pe_vol < CONFIG["MIN_VOLUME"]:
            return None
        ...
        if ce_entry <= 0 or pe_entry <= 0:
            return None
        if ce_entry < CONFIG["MIN_OPTION_PREMIUM"] or pe_entry < CONFIG["MIN_OPTION_PREMIUM"]:
            return None
        ...
        if ce_spread > CONFIG["MAX_SPREAD_PCT"] or pe_spread > CONFIG["MAX_SPREAD_PCT"]:
            return None
        
        total_credit = ce_entry + pe_entry
        confidence = min(100, (percentile - CONFIG["SELL_ZONE"]) * 2 + 50)
        ...
        return {
            ...
            "sl": round(total_credit * CONFIG["SELL_SL_MULTIPLIER"], 2),
            "target": round(total_credit * CONFIG["SELL_TARGET_MULTIPLIER"], 2),
        }
    ...
    def _build_buy_signal(self, security_id, symbol, spot, current_iv, percentile, zscore, trend):
        ...
        if entry <= 0:
            return None
        if entry < CONFIG["MIN_OPTION_PREMIUM"]:
            return None
        ...
        confidence = min(100, (CONFIG["BUY_ZONE"] - percentile) * 2 + 50)
        ...
        return {
            ...
            "oi": oi,
            "volume": vol,
            "sl": round(entry * CONFIG["BUY_SL_MULTIPLIER"], 2),
            "target": round(entry * CONFIG["BUY_TARGET_MULTIPLIER"], 2),
        }
    ...
    def _get_expiry(self, security_id, segment):
        ...
        for expiry in expiries:
            if self.scanner.days_to_expiry(expiry) >= CONFIG["DTE_MIN"]:
                return expiry
    ...
    def scan_all(self, securities: dict) -> list:
        ...
        for sec_id, symbol in securities.items():
            # Check daily limit
            if self._signals_today >= CONFIG["MAX_SIGNALS_PER_DAY"]:
                break
        ...
        return results[:CONFIG["MAX_SIGNALS_PER_DAY"]]
    
    def run_paper_trade(self, signal: dict) -> dict:
        """Book a paper trade from a signal."""
        try:
            # Build trade dict for paper_trader
            if signal["direction"] == "short":
                # For strangle, we need to book both legs
                # paper_trader expects one trade with a combo_id
                trade = {
                    ...
                    "dte": signal["dte"],
                    "spot": signal["spot"],
                    "iv_percentile": signal["percentile"],
                    ...
                    "entry": signal["total_credit"],  # For paper_trader
                }
            else:
                # For long option
                trade = {
            ...
            # Book via paper_trader
            result = book_signal(trade)
            
            # Send alert
            notifications.notify(f"""
📊 IV PERCENTILE TRADE BOOKED
