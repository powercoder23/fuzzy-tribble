...
class DiscountedPremiumScanner:
    ...
    def send_telegram_summary(self, opportunities_df):
        ...
        if opportunities_df is None or opportunities_df.empty:
            ...
            valid_pcr = opportunities_df["pcr"].dropna() if "pcr" in opportunities_df.columns else pd.Series(dtype=float)
            valid_skew = opportunities_df["skew_ratio"].dropna() if "skew_ratio" in opportunities_df.columns else pd.Series(dtype=float)
            lines = [
                "Options Scanner Summary",
                f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                f"Matches: {len(opportunities_df)}",
                f"PCR: {valid_pcr.median():.2f}" if not valid_pcr.empty else "PCR: N/A",
                f"Skew (CE/PE IV): {valid_skew.median():.2f}" if not valid_skew.empty else "Skew: N/A",
            ]
            ...
            for strategy_name, strategy_rows in grouped_rows:
                ...
                for _, row in strategy_rows.head(5).iterrows():
                    lines.append(
                        ...
                        f"Score {row['score']:.1f} | IV {row['iv']:.1f} | Delta {abs(row['delta']):.2f}"
                    )
            message = "\n".join(lines)
    ...
    def compute_market_bias(self, pcr, skew_ratio, ce_volume, pe_volume, trend):
        """Combine positioning signals into a directional market bias score."""
        bias_score = 0.0
        ...
        if ce_volume is not None and pe_volume is not None:
            if ce_volume > 0 and pe_volume > 0:
                volume_ratio = ce_volume / pe_volume
            elif ce_volume > 0:
        ...
        return round(max(-5.0, min(5.0, bias_score)), 2)
    ...
    def scan_single_strike(self, strike_data, strike_price, spot_price, option_chain,
                          ...
                          call_avg_volume=None, put_avg_volume=None, call_avg_oi=None,
                          put_avg_oi=None, iv_behavior=None,
                          premarket_ctx=None, market_context=None):
        """
        ...
        for option_type in ['ce', 'pe']:
            ...
            volume_ratio = (volume / avg_peer_volume) if avg_peer_volume and avg_peer_volume > 0 else None
            oi_ratio = (oi / avg_peer_oi) if avg_peer_oi and avg_peer_oi > 0 else None
            quality_score = 0
            ...
            if volume_ratio is not None and volume_ratio > 1.2:
                ...
                quality_score += 1
            if oi_ratio is not None and oi_ratio >= 1.0:
                quality_score += 1
            ...
            iv_change = premarket_ctx.get("iv_change") if premarket_ctx else None
            iv_state = "neutral"
            if iv_change is not None:
                if iv_change < -2:
                    ...
                    iv_state = "expanded"
                logger.info("IV State: %s | IV Change: %.2f", iv_state, iv_change)
                if iv_change > 3:
                    continue
            ...
            if volume_ratio is not None and volume_ratio > 1.5:
                if oi_ratio is not None and oi_ratio > 1.2:
                    score_adjustment += 10.0
                elif oi_ratio is not None and oi_ratio < 0.8:
            ...
            pcr = (market_context or {}).get("pcr")
            skew_ratio = (market_context or {}).get("skew_ratio")
            market_bias_score = (market_context or {}).get("market_bias_score", 0.0)
            market_bias = (market_context or {}).get("market_bias", "neutral")
            directional_adjustment = 0.0
            iv_timing_adjustment = 0.0

            option_matches_bias = (
                (market_bias == "bullish" and option_type == "ce") or
                (market_bias == "bearish" and option_type == "pe")
            )

            if iv_state == "compressed":
                iv_timing_adjustment += 10.0
            elif iv_state == "expanded":
                iv_timing_adjustment -= 6.0 if option_matches_bias else 15.0

            if market_bias_score > 2:
                if iv_state == "compressed":
                    iv_timing_adjustment += 6.0 if option_type == "ce" else 2.0
                elif iv_state == "expanded":
                    iv_timing_adjustment -= 8.0 if option_type == "ce" else 12.0
            elif market_bias_score < -2:
                if iv_state == "compressed":
                    iv_timing_adjustment += 6.0 if option_type == "pe" else 2.0
                elif iv_state == "expanded":
                    iv_timing_adjustment -= 8.0 if option_type == "pe" else 12.0

            if abs(market_bias_score) > 2:
                bias_adjustment = 8.0 + (min(abs(market_bias_score), 5.0) * 2.0)
                if market_bias_score > 2:
                    directional_adjustment += bias_adjustment if option_type == "ce" else -bias_adjustment
                elif market_bias_score < -2:
                    directional_adjustment += bias_adjustment if option_type == "pe" else -bias_adjustment

            score = round(clip_score(score + iv_timing_adjustment + directional_adjustment), 2)
            ...
            if volume_ratio is not None and volume_ratio > 2.0:
                reasons.append(f"Volume is {volume_ratio:.2f}x the same-side chain average")
            elif volume_ratio is not None and volume_ratio > 1.5:
                reasons.append(f"Volume is meaningfully above peer average at {volume_ratio:.2f}x")
            if volume_ratio is not None and volume_ratio > 1.5 and oi_ratio is not None and oi_ratio > 1.2:
                reasons.append("High volume and high OI signal strong participation")
            elif volume_ratio is not None and volume_ratio > 1.5 and oi_ratio is not None and oi_ratio < 0.8:
                reasons.append("High volume with lighter OI suggests fresh positioning")
            if iv_change is not None:
                reasons.append(f"Premarket IV state is {iv_state} ({iv_change:.2f})")
            ...
            if abs(market_bias_score) > 2:
                if market_bias_score > 2 and option_type == "ce":
                    reasons.append(f"Market bias score {market_bias_score:.2f} favors calls")
                elif market_bias_score < -2 and option_type == "pe":
                    reasons.append(f"Market bias score {market_bias_score:.2f} favors puts")
            ...
            discounted.append({
                ...
                "pcr": native_number(pcr),
                "skew_ratio": native_number(skew_ratio),
                "market_bias_score": native_number(market_bias_score),
    ...
    def scan_underlying(self, security_id, security_segment, security_name, 
        ...
        for strike_data in option_chain.values():
            ...
            if pd.notna(call_oi) and call_oi > 0:
                call_ois.append(float(call_oi))
            if pd.notna(put_oi) and put_oi > 0:
                put_ois.append(float(put_oi))
        ...
        ce_volume_total = float(sum(call_volumes)) if call_volumes else 0.0
        pe_volume_total = float(sum(put_volumes)) if put_volumes else 0.0
        call_avg_oi = float(np.mean(call_ois)) if call_ois else None
        put_avg_oi = float(np.mean(put_ois)) if put_ois else None
        total_call_oi = sum(
            (strike.get("ce") or {}).get("oi", 0)
            for strike in option_chain.values()
            if isinstance(strike, dict)
        )
        total_put_oi = sum(
            (strike.get("pe") or {}).get("oi", 0)
            for strike in option_chain.values()
            if isinstance(strike, dict)
        )
        pcr = total_put_oi / total_call_oi if total_call_oi > 0 else None
        skew_ratio = None
        if call_mean and put_mean and put_mean > 0:
            skew_ratio = call_mean / put_mean
        market_bias_score = self.compute_market_bias(
            pcr=pcr,
            skew_ratio=skew_ratio,
            ce_volume=ce_volume_total,
            pe_volume=pe_volume_total,
            trend="neutral",
        )
        market_bias = "neutral"
        if market_bias_score >= 1.5:
            market_bias = "bullish"
        elif market_bias_score <= -1.5:
            market_bias = "bearish"
        market_context = {
            "pcr": pcr,
            "skew_ratio": skew_ratio,
        ...
        logger.info(
            "Market Context: PCR=%s | CE/PE Skew=%s | CE Vol=%s | PE Vol=%s | Bias=%s (%.2f)",
            f"{pcr:.2f}" if pcr is not None else "N/A",
            f"{skew_ratio:.2f}" if skew_ratio is not None else "N/A",
            f"{ce_volume_total:.0f}",
            f"{pe_volume_total:.0f}",
            market_bias,
            market_bias_score,
        )
        ...
        for strike_str, strike_data in option_chain.items():
            ...
            discounted = self.scan_single_strike(
                ...
                call_avg_oi=call_avg_oi,
                put_avg_oi=put_avg_oi,
        ...
        calls = sorted([opt for opt in all_discounted if opt["type"] == "CALL"], key=lambda x: x["score"], reverse=True)
        puts = sorted([opt for opt in all_discounted if opt["type"] == "PUT"], key=lambda x: x["score"], reverse=True)
        target_count = 10
        if market_bias_score >= 1.5:
            call_target = math.ceil(target_count * 0.7)
        elif market_bias_score <= -1.5:
            call_target = math.floor(target_count * 0.3)
        else:
            call_target = target_count // 2
        put_target = target_count - call_target

        selected = calls[:call_target] + puts[:put_target]
        remaining_slots = target_count - len(selected)
        if remaining_slots > 0:
            selected_ids = {id(opt) for opt in selected}
            remaining_pool = sorted(
                [opt for opt in all_discounted if id(opt) not in selected_ids],
                key=lambda x: x["score"],
                reverse=True,
            )
            selected.extend(remaining_pool[:remaining_slots])

        all_discounted = sorted(selected, key=lambda x: x["score"], reverse=True)[:target_count]
        logger.info(
            "Underlying %s opportunities | before_stock_cap=%s | after_stock_cap=%s | calls=%s | puts=%s | bias=%s (%.2f)",
            security_name,
            before_underlying_cap,
            len(all_discounted),
            sum(1 for opt in all_discounted if opt["type"] == "CALL"),
            sum(1 for opt in all_discounted if opt["type"] == "PUT"),
            market_bias,
            market_bias_score,
        )
