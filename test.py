...
class DiscountedPremiumScanner:
    ...
    def __init__(self, hardtoken, client_id="1104878989", store_intraday=False):
        ...
        # FNO Stock Security IDs (partial list - you can expand)
        self.fno_stocks = {
            13: "NIFTY",           # Index
            14: "BANKNIFTY",       # Index
            1333: "HDFCBANK",      # Stock
            1592: "RELIANCE",      # Stock
            1610: "ICICIBANK",     # Stock
            1523: "INFY",          # Stock
            1394: "TCS",           # Stock
            1510: "HINDUNILVR",    # Stock
            1633: "SBIN",          # Stock
            # Add more FNO stocks as needed
        }
    ...
    def score_option(self, current_iv, weighted_hv, delta, vega, oi, volume, skew_discount,
                     expected_move_ratio, iv_rank, iv_percentile):
    ...
    def scan_single_strike(self, strike_data, strike_price, spot_price, option_chain,
        ...
        for option_type in ['ce', 'pe']:
            ...
            iv_rank = self.calculate_iv_rank(current_iv, historical_ivs)
    ...
    def scan_underlying(self, security_id, security_segment, security_name, 
        ...
        iv_rank_atm = self.calculate_iv_rank(atm_context.get("atm_iv") or 0, historical_ivs) if atm_context.get("atm_iv") else None
        iv_percentile_atm = self.calculate_iv_percentile(atm_context.get("atm_iv") or 0, historical_ivs) if atm_context.get("atm_iv") else None
        ...
        if iv_rank_atm is not None and historical_ivs:
    ...
    def generate_report(self, opportunities_df):
        ...
        for _, row in opportunities_df.iterrows():
            ...
            iv_rank_text = f"{row['iv_rank']:.2f}" if pd.notna(row['iv_rank']) else "N/A"
            iv_percentile_text = f"{row['iv_percentile']:.2f}" if pd.notna(row['iv_percentile']) else "N/A"
            ...
            logger.info("%s", "-" * 50)
            logger.info("Score: %.2f/100 | Type: %s", row['score'], row['type'])
            logger.info("IV: %.2f%% | IV Rank: %s | IV Percentile: %s", row['iv'], iv_rank_text, iv_percentile_text)
