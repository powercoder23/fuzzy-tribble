...
class ScripMasterLotSizer:
    ...
    def _load_all(self) -> None:
        ...
        try:
            ...
            # Lot sizes are revised over time, so the scrip master holds several
            # expiries with DIFFERENT lots for the same underlying (e.g. GAIL
            # 3150 near / 3550 far). Pick the lot of the NEAREST ACTIVE expiry
            # (front month) — not min/max, which were both wrong in different
            # cases. Selection key (lower = better):
            #   (0, days_to_expiry)  for expiries >= today  (nearest active wins)
            #   (1, age_in_days)     for past expiries       (most recent wins)
            #   (2, 0)               when expiry is unparseable (last resort)
            today = date.today()
            best_key: dict[str, tuple] = {}
            for trading_sym, lot_size, expiry_raw in rows:
                ...
                if expiry_raw:
                    try:
                        expd = datetime.strptime(str(expiry_raw)[:10], "%Y-%m-%d").date()
                    except (ValueError, TypeError):
                        expd = None
                if expd is None:
                    key = (2, 0)
                elif expd >= today:
                    key = (0, (expd - today).days)
                else:
                    key = (1, (today - expd).days)
                if underlying not in best_key or key < best_key[underlying]:
                    best_key[underlying] = key
