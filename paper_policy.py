...
---------------------------------------------------------
On 2026-08-05, with every strategy nominally profile-gated off in
docker-compose, the book still took 51 trades from vol-expansion,
convex-engine and break-and-bounce. Two structural reasons, neither fixable
per-strategy:
  ...
  * Several strategies had no paper on/off flag of their own at all — they
    called OrderManager.submit_external_signal unconditionally, so there was
    nothing to turn off.
...
variants: "IV_" allows "IV_SELL_STRANGLE (IVP 87.0%)", "IV_BUY_CE (IVP 12.0%)"
and "IV_BUY_PE (IVP 12.0%)" — the iv_percentile_strategy.py tags.

  * default: `IV_` (2026-08-06 — every other strategy was removed outright;
    iv-percentile is the sole strategy left, so it is the sole default entry)
